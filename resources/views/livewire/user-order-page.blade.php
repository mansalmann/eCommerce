                ...
                @forelse ($orders as $order)
                  ...
                  <tr class="odd:bg-white even:bg-gray-100 dark:odd:bg-slate-900 dark:even:bg-slate-800" wire:key="{{ $order->id }}">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                      <a href="{{ route('user-detail-order', ['orderId' => $order->id]) }}" class="bg-slate-600 text-white py-2 px-4 rounded-md hover:bg-slate-500">View Details</a>
                    </td>
                  </tr>
                @empty
                <tr>
                  <td colspan="6" class="text-center py-4 text-4xl font-semibold text-slate-500">No order records.</td>
                </tr>
                @endforelse
              </tbody>
            </table>
